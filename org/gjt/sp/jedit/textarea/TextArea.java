 ...
 *
 * This class uses a minimal set of jEdit APIs because it is the base class of
 * the JEditEmbeddedTextArea and StandaloneTextArea, so it needs to be
 * embeddable and separable.
 *
 * @author Slava Pestov
...
public abstract class TextArea extends JComponent {
	 ...
	 *            the property manager that contains informations like shortcut
	 *            bindings
	 * @param inputHandlerProvider
	 *            the inputHandlerProvider
	 */
	protected TextArea(IPropertyManager propertyManager,
		...
		gutter.setMouseActionsProvider(new MouseActions(propertyManager,
				"gutter"));
		...
		add(ScrollLayout.BOTTOM, horizontal = new JScrollBar(
	 ...
	 * textarea. Called when creating a standalone textarea from within jEdit.
	 */
	public void initInputHandler() {
		actionContext = new JEditActionContext<JEditBeanShellAction, JEditActionSet<JEditBeanShellAction>>() {
			...
			public void invokeAction(EventObject evt,
					JEditBeanShellAction action) {
				action.invoke(TextArea.this);
			}
		};

		setMouseHandler(new TextAreaMouseHandler(this));
		inputHandlerProvider = new DefaultInputHandlerProvider(
				new TextAreaInputHandler(this) {
					@Override
	...
	public void setTransferHandler(TransferHandler newHandler) {
		...
		try {
			getDropTarget()
					.addDropTargetListener(new TextAreaDropHandler(this));
	...
	// {{{ setBuffer() method
	/**
	 * Sets the buffer this text area is editing. If you don't run a standalone
	 * textarea in jEdit please do not call this method - use
	 * {@link org.gjt.sp.jedit.EditPane#setBuffer(org.gjt.sp.jedit.Buffer)}
	 * instead.
	 * 
	 * @param buffer
	 *            The buffer
	 ...
	 * called by the org.gjt.sp.jedit.Java14 class to signal that a drag is in
	 * progress.
	 * 
	 * @since jEdit 4.2pre5
	 ...
	 * @since jEdit 4.3pre10
	 */
	public void setCtrlForRectangularSelection(
	...
	public void setFirstLine(int firstLine) {
		...
		if (Debug.SCROLL_DEBUG) {
			Log.log(Log.DEBUG, this, "setFirstLine() from " + getFirstLine()
					+ " to " + firstLine);
		}
	...
	public void setFirstPhysicalLine(int physFirstLine, int skew) {
		if (Debug.SCROLL_DEBUG) {
			Log.log(Log.DEBUG, this, "setFirstPhysicalLine(" + physFirstLine
					+ ',' + skew + ')');
		}
	...
	public void scrollTo(int line, int offset, boolean doElectricScroll) {
		...
		int _electricScroll = doElectricScroll
				&& visibleLines - 1 > (electricScroll << 1) ? electricScroll
				: 0;
		// }}}
		...
		if (screenLine == -1) {
			...
			int prevLine = displayManager
					.getPrevVisibleLine(getFirstPhysicalLine());
			int nextLine = displayManager
					.getNextVisibleLine(getLastPhysicalLine());
			if (line == getFirstPhysicalLine()) {
				...
				if (Debug.SCROLL_TO_DEBUG)
					Log.log(Log.DEBUG, this, line + " == "
							+ getLastPhysicalLine());
				setFirstPhysicalLine(line, subregion + _electricScroll
						- visibleLines + (lastLinePartial ? 2 : 1));
			} else if (line == nextLine) {
				if (Debug.SCROLL_TO_DEBUG)
					Log.log(Log.DEBUG, this, line + " == " + nextLine);
				setFirstPhysicalLine(nextLine, subregion + _electricScroll
						- visibleLines + (lastLinePartial ? 2 : 1));
			} else {
				if (Debug.SCROLL_TO_DEBUG) {
					Log.log(Log.DEBUG, this, "neither");
					Log.log(Log.DEBUG, this, "Last physical line is "
							+ getLastPhysicalLine());
				}
			...
			setFirstLine(getFirstLine() + _electricScroll - visibleLines
					+ screenLine + (lastLinePartial ? 2 : 1));
		} // }}}
		...
		if (point.x < 0) {
			...
			setHorizontalOffset(horizontalOffset
					+ (painter.getWidth() - point.x) - charWidth - 5);
		} // }}}
	} // }}}
	...
	public int xToScreenLineOffset(int screenLine, int x, boolean round) {
		...
		if (lineInfo.physicalLine == -1) {
			...
			int offset = Chunk.xToOffset(lineInfo.chunks, x - horizontalOffset,
					round);
	...
	public Point offsetToXY(int line, int offset, Point retVal) {
		...
		retVal.x = (int) (horizontalOffset + Chunk.offsetToX(info.chunks,
				offset));
	...
	public void invalidateLine(int line) {
		if (!isShowing() || buffer.isLoading() || line < getFirstPhysicalLine()
				|| line > physLastLine || !displayManager.isLineVisible(line))
			return;
	...
	/**
	 * Copies the text on the specified line into a Segment. If lineIndex is
	 * invalid, the segment will contain a null string.
	 * 
	 * @param lineIndex
	 *            The line number (physical line)
	 * @param segment
	 *            the segment into which the data will be stored.
	 */
	...
	public void selectWord() {
		...
		Selection s = new Selection.Range(lineStart + wordStart, lineStart
				+ wordEnd);
		if (multi)
			addToSelection(s);
		else
	 ...
	 * @since jEdit 4.2pre1
	 */
	public void extendSelection(int offset, int end, int extraStartVirt,
			int extraEndVirt) {
	...
	public int getMagicCaretPosition() {
		if (magicCaret == -1) {
			magicCaret = chunkCache.subregionOffsetToX(caretLine, caret
	...
	public void goToNextCharacter(boolean select) {
		...
		if (caret == buffer.getLength()) {
			if (select
					&& (rectangularSelectionMode || s instanceof Selection.Rect)) {
	...
	public void goToNextWord(boolean select, boolean eatWhitespace) {
		...
		if (newCaret == lineText.length()) {
			...
			String noWordSep = buffer.getStringProperty("noWordSep");
			boolean camelCasedWords = buffer
					.getBooleanProperty("camelCasedWords");
	...
	// {{{ home() method
	/**
	 * A "dumb home" action which only has 2 states: start of the whitespace or
	 * start of line
	 * 
	 * @param select
	 *            true if we also want to select from the cursor
	...
	// {{{ end() method
	/**
	 * a dumb end action which only has 2 states: end of whitespace or end of
	 * line
	 * 
	 * @param select
	 *            true if we also want to select from the cursor
	...
	// {{{ smartEnd() method
	/**
	 * Has 4 states based on # of invocations: 1. last character of code (before
	 * inline comment) 2. last non whitespace character of the line 3. end of
	 * line 4. end of last visible line
	 * 
	 * @param select
	 *            true if you want to extend selection
	 * @since jEdit 4.3pre18
	...
	/**
	 * Moves the caret to the end of the code present on the current line,
	 * before the comments and whitespace.
	 * 
	 * @param select
	 *            true if you want to extend selection
	 * @since jEdit 4.3pre18
	...
	public void goToEndOfCode(boolean select) {
		...
		while (true) {
			if (token.id == Token.COMMENT1 || token.id == Token.COMMENT2
					|| token.id == Token.COMMENT3 || token.id == Token.COMMENT4) {
		...
		int newCaret = getLineLength(line)
				- StandardUtilities.getTrailingWhiteSpace(new String(txt));
		newCaret += getLineStartOffset(line);
	...
	public void goToStartOfWhiteSpace(boolean select) {
		...
		if (firstIndent == getLineStartOffset(line)) {
			firstIndent = StandardUtilities
					.getLeadingWhiteSpace(getLineText(line));
	...
	public void goToEndOfWhiteSpace(boolean select) {
		...
		if (lastIndent == getLineEndOffset(line)) {
			lastIndent = getLineLength(line)
					- StandardUtilities
							.getTrailingWhiteSpace(getLineText(line));
			if (lastIndent == 0)
				lastIndent = getLineLength(line);
	...
	public void goToLastVisibleLine(boolean select) {
		...
		if (getFirstLine() + visibleLines >= displayManager
				.getScrollLineCount()) {
	...
	public void goToBufferStart(boolean select) {
		int start = buffer.getLineStartOffset(displayManager
				.getFirstVisibleLine());
		if (select)
	...
	public void deleteLine() {
		...
		int x = chunkCache.subregionOffsetToX(caretLine, caret
				- getLineStartOffset(caretLine));
	...
	public void deleteToStartOfLine() {
		...
		buffer.remove(getLineStartOffset(caretLine), caret
				- getLineStartOffset(caretLine));
	} // }}}
	...
	public void deleteWord(boolean eatWhitespace) {
		...
		if (_caret == lineText.length()) {
			...
			boolean camelCasedWords = buffer
					.getBooleanProperty("camelCasedWords");
			_caret = TextUtilities.findWordEnd(lineText, _caret + 1, noWordSep,
					true, camelCasedWords, eatWhitespace);
		}
	...
	public void selectFold(int line) {
		...
		Selection s = new Selection.Range(getLineStartOffset(lines[0]),
				newCaret);
		if (multi)
			addToSelection(s);
		else
	 ...
	 *             an exception thrown if the folding mode is not explicit
	 * @since jEdit 4.0pre3
	 */
		...
		try {
			...
			if (getSelectionCount() == 0) {
				...
				for (int i = 0; i < selections.length; i++) {
					...
					caretBack = addExplicitFold(selection.start, selection.end,
	...
	public void lineComment() {
		...
		String comment = buffer.getContextSensitiveProperty(caret,
				"lineComment");
	...
	public void rangeComment() {
		String commentStart = buffer.getContextSensitiveProperty(caret,
				"commentStart");
		String commentEnd = buffer.getContextSensitiveProperty(caret,
				"commentEnd");
	...
	public void formatParagraph() throws TextAreaException {
		...
		if (selection.length != 0) {
			...
			for (int i = 0; i < selection.length; i++) {
				Selection s = selection[i];
				setSelectedText(s, TextUtilities.format(getSelectedText(s),
						maxLineLen, buffer.getTabSize()));
			}
			...
			try {
				...
				int noSpaceOffset = TextUtilities.indexIgnoringWhitespace(text,
						offset);
				buffer.remove(start, end - start);
				text = TextUtilities.format(text, maxLineLen,
						buffer.getTabSize());
				buffer.insert(start, text);
				int caretPos = start;
				if (text.length() != 0) {
					caretPos += Math.min(text.length(), TextUtilities
							.ignoringWhitespaceIndex(text, noSpaceOffset));
				}
	...
	public void spacesToTabs() {
		...
		if (selection.length == 0) {
			...
			for (int i = 0; i < selection.length; i++) {
				...
				setSelectedText(
						s,
						TextUtilities.spacesToTabs(getSelectedText(s),
								buffer.getTabSize()));
			}
		}
	...
	public void tabsToSpaces() {
		...
		if (selection.length == 0) {
			...
			for (int i = 0; i < selection.length; i++) {
				...
				setSelectedText(
						s,
						TextUtilities.tabsToSpaces(getSelectedText(s),
								buffer.getTabSize()));
			}
		}
	...
	// {{{ changeCase(String toCase) method
	/**
	...
	// amoyeen: modified to remove code duplication	
	// {{{ toUpperCase() method
	/**
	...
	/**
	 * Adds a component to the left side of the box left of the vertical scroll
	 * bar. The ErrorList plugin uses this to show a global error overview, for
	 * example. It is possible for more than one component to be added, each is
	 * added to the left side of the box in turn. Adding to the left ensures the
	 * scrollbar is always right of all added components.
	 *
	 * @param comp
	 *            The component
	 * @since jEdit 4.2pre1
	...
	public void addNotify() {
		...
		hiddenCursor = getToolkit().createCustomCursor(
				getGraphicsConfiguration().createCompatibleImage(16, 16,
						Transparency.BITMASK), new Point(0, 0), "Hidden");
	} // }}}
	...
	public void processKeyEvent(KeyEvent evt) {
		getInputHandler().processKeyEvent(evt, 1 /* source=TEXTAREA (1) */,
				false);
		if (!evt.isConsumed())
	...
	public void propertiesChanged() {
		...
		charWidth = (int) Math.round(painter.getFont()
				.getStringBounds(foo, 0, 1, painter.getFontRenderContext())
				.getWidth());
		...
		boolean wrapSettingsChanged = !(wrap.equals(oldWrap)
				&& oldWrapToWidth == wrapToWidth && oldWrapMargin == wrapMargin);

		if (displayManager != null && !bufferChanging && !buffer.isLoading()
				&& wrapSettingsChanged) {
	...
	// {{{ addActionSet() method
	/**
	 * Adds a new action set to the textarea's list of ActionSets. Call this
	 * only on standalone textarea
	 *
	 * @param actionSet
			...
			horizontal
					.setValues(Math.max(
							...
							Math.min(maxHorizontalScrollWidth + charWidth
									- painter.getWidth(), -horizontalOffset)),
							painter.getWidth(), 0, maxHorizontalScrollWidth
									+ charWidth);
	...
	void _finishCaretUpdate() {
		...
		try {
			...
			int newCaretScreenLine = chunkCache.getScreenLineOfOffset(
					caretLine, caret - buffer.getLineStartOffset(caretLine));
			if (caretScreenLine == -1)
				invalidateScreenLineRange(newCaretScreenLine,
						newCaretScreenLine);
			else
	...
	void startDragAndDrop(InputEvent evt, boolean copy) {
		...
		if (transferHandler != null) {
			...
			transferHandler.exportAsDrag(this, evt, copy ? TransferHandler.COPY
	...
	/**
	 * Sets the horizontal offset of drawn lines. This method will check if the
	 * offset do not go too far after the last character
	 * 
	 * @param horizontalOffset
	 *            offset The new horizontal offset
	 */
	private void _setHorizontalOffset(int horizontalOffset) {
		...
		int min = Math
				.min(-(maxHorizontalScrollWidth + charWidth - painter
						.getWidth()), 0);
	 ...
	 */
	private void finishCaretUpdate(int oldCaretLine, int scrollMode,
	...
	private void fireScrollEvent(boolean vertical) {
		...
		for (int i = listeners.length - 2; i >= 0; i--) {
			if (listeners[i] == ScrollListener.class) {
				try {
					if (vertical)
						((ScrollListener) listeners[i + 1])
								.scrolledVertically(this);
					else
						((ScrollListener) listeners[i + 1])
								.scrolledHorizontally(this);
				} catch (Throwable t) {
					Log.log(Log.ERROR, this, t);
				}
			}
	...
	private void fireStatusChanged(int flag, boolean value) {
		...
		for (int i = listeners.length - 2; i >= 0; i--) {
			if (listeners[i] == StatusListener.class) {
				try {
					((StatusListener) listeners[i + 1]).statusChanged(this,
							flag, value);
				} catch (Throwable t) {
					Log.log(Log.ERROR, this, t);
				}
	...
	private void fireBracketSelected(int line, String text) {
		...
		for (int i = listeners.length - 2; i >= 0; i--) {
			if (listeners[i] == StatusListener.class) {
				try {
					((StatusListener) listeners[i + 1]).bracketSelected(this,
							line, text);
				} catch (Throwable t) {
					Log.log(Log.ERROR, this, t);
				}
	...
	private void insertTab() {
		...
		if (buffer.getBooleanProperty("noTabs")) {
			...
			replaceSelection(StandardUtilities.createWhiteSpace(tabSize - pos,
					0));
		} else
			replaceSelection("\t");
	} // }}}
	...
	protected void userInputTab() {
		if (getSelectionCount() == 1) {
			...
			if (sel instanceof Selection.Rect
					|| (sel.startLine == sel.endLine && (sel.start != buffer
							.getLineStartOffset(sel.startLine) || sel.end != buffer
	...
	protected boolean doWordWrap(boolean spaceInserted) {
		...
		for (int i = 0; i < caretPos; i++) {
			...
			} else if (wordBreakChars != null
					&& wordBreakChars.indexOf(ch) != -1) {
				logicalLength++;
				if (!lastWasSpace && logicalLength <= maxLineLen) {
					lastInLine = i;
					lastWordOffset = i;
	...
	private void updateStructureHighlight() {
		...
		if (match != null) {
			...
			if (!displayManager.isLineVisible(match.startLine)
					|| chunkCache.getScreenLineOfOffset(match.startLine,
							match.start - getLineStartOffset(match.startLine)) == -1) {
	...
	private void showStructureStatusMessage(boolean backward) {
		...
		if (backward && match.startLine != 0 && text.length() == 1) {
			...
			case '(':
			case ')':
				text = buffer.getLineText(match.startLine - 1).trim() + ' '
						+ text;
				break;
			}
		}
	...
	private RectParams getRectParams(int caret, int newCaret) {
		...
		if (s instanceof Selection.Rect) {
			if (caret == s.end) {
				virtualWidth = buffer.getVirtualWidth(s.endLine, s.end
						- getLineStartOffset(s.endLine))
						+ ((Selection.Rect) s).extraEndVirt;
			} else {
				virtualWidth = buffer.getVirtualWidth(s.startLine, s.start
						- getLineStartOffset(s.startLine))
						+ ((Selection.Rect) s).extraStartVirt;
			}
		} else if (rectangularSelectionMode) {
			virtualWidth = buffer.getVirtualWidth(caretLine,
		...
		int newOffset = buffer.getOffsetOfVirtualColumn(newLine, virtualWidth,
				totalVirtualWidth);
		if (newOffset == -1) {
	...
	private void tallCaretDelete(Selection.Rect s, boolean forward) {
		try {
			...
			for (int i = startLine; i <= endLine; i++) {
				int offset = buffer
						.getOffsetOfVirtualColumn(i, startCol, width);
				if (offset == -1) {
					if (width[0] == startCol)
	...
	private void shiftTallCaretLeft(Selection.Rect s) {
		removeFromSelection(s);
		addToSelection(new Selection.Rect(buffer, s.getStartLine(),
				s.getStartColumn(buffer) - 1, s.getEndLine(),
	...
	private void setMaxLineLength(int maxLineLen) {
		...
		if (maxLineLen <= 0) {
			...
			wrapToWidth = false;
			wrapMargin = (int) painter
					.getFont()
					.getStringBounds(foo, 0, foo.length,
	 ...
	 * @since jEdit 4.3pre3
	 */
	protected int addExplicitFold(int caretStart, int caretEnd, int lineStart,
			int lineEnd) {
		...
		// in HTML code)
		int startCaret = caretStart < buffer.getLength() ? caretStart + 1
				: caretStart;
		int endCaret = caretEnd > 0 ? caretEnd - 1 : caretEnd;

		String startLineComment = buffer.getContextSensitiveProperty(
				startCaret, "lineComment");
		String startCommentStart = buffer.getContextSensitiveProperty(
				startCaret, "commentStart");
		String startCommentEnd = buffer.getContextSensitiveProperty(startCaret,
				"commentEnd");
		String endLineComment = buffer.getContextSensitiveProperty(endCaret,
				"lineComment");
		String endCommentStart = buffer.getContextSensitiveProperty(endCaret,
				"commentStart");
		String endCommentEnd = buffer.getContextSensitiveProperty(endCaret,
				"commentEnd");
	...
	/**
	 * This method will surround each selected line with a range comment. This
	 * is used when calling line comment if the edit mode doesn't have a line
	 * comment property
	 * 
	 * @since jEdit 4.3pre10
	 */
		...
		try {
			...
			for (int i = 0; i < lines.length; i++) {
				...
				if (text.trim().length() == 0)
					continue;
				buffer.insert(getLineEndOffset(lines[i]) - 1, commentEnd);
				buffer.insert(
						getLineStartOffset(lines[i])
								+ StandardUtilities.getLeadingWhiteSpace(text),
						commentStart);
	...
	// {{{ joinLine() method
	/**
	 * Join a line with the next line. If you use this method you have to lock
	 * the buffer in compound edit mode.
	 * 
	 * @param line
	 *            the line number that will be joined with the next line
	 */
	private void joinLineAt(int line) {
		...
		buffer.remove(end - 1,
				StandardUtilities.getLeadingWhiteSpace(nextLineText) + 1);
	...
	/**
	 * Returns if the right click popup menu is enabled. The Gestures plugin
	 * uses this API.
	 * 
	...
	/**
	 * Sets if the right click popup menu is enabled. The Gestures plugin uses
	 * this API.
	 * 
	 * @since jEdit 4.2pre13
	 */
	...
	/**
	 * Do the same thing as right-clicking on the text area. The Gestures plugin
	 * uses this API.
	 * 
	 * @since jEdit 4.2pre13
	 */
	...
	public void showPopupMenu() {
		if (!popup.isVisible() && hasFocus()) {
			...
			if (caretPos != null) {
				...
				showPopupMenu(popup, painter, caretPos.x, caretPos.y
						+ charHeight, true);
			}
		}
	} // }}}
	 ...
	 * @since jEdit 4.1pre1
	 */
	private static void showPopupMenu(JPopupMenu popup, Component comp, int x,
			int y, boolean point) {
		int offsetX = 0;
		int offsetY = 0;
	...
	private class MouseWheelHandler implements MouseWheelListener {
		public void mouseWheelMoved(MouseWheelEvent e) {
			 ...
			 * shift: scroll horizontally - control: scroll single line -
			 * <else>: scroll 3 lines
			 ****************************************************/
			if (e.isAltDown()) {
